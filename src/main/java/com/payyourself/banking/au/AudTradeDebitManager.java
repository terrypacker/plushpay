...
public class AudTradeDebitManager extends Thread{
	...
	private static AudTradeDebitManager singleton;
	
	private CurrencyConverter conv;
	
	public static AudTradeDebitManager getAudTradeDebitManager(){
		if (AudTradeDebitManager.singleton == null)
			// it's ok, we can call this constructor
			AudTradeDebitManager.singleton = new AudTradeDebitManager();		
		return singleton;
	}
	...
	public void processTradeDebits() throws Exception {
		...
		for(int i=0; i<trades.size(); i++){
			...
			if(trades.get(i).getBuyerGroup().getCurrencyToBuy().getType().getCode() == CurrencyCodeEnum.AUD){
				traders = trades.get(i).getBuyerGroup().getTraders(); 
			}else if(trades.get(i).getSellerGroup().getCurrencyToBuy().getType().getCode() == CurrencyCodeEnum.AUD){
		...
		if(debits.size()>0){
			...
			NabFileTotalRecord fileTotal = new NabFileTotalRecord(AudBankSimulator.getBsbNumber(), totalDebits, totalDebits, 0, debits.size());

...
public class GeneticTradeManager extends Thread{
	...
	private void writeStats(Creature best) throws IOException {
		...
		if(this.writeToFile){
			String data = this.apocalypse + "," + best.getRoi() + ","+ best.getGain()  + "," + best.getCost() + "," +
				 + best.getBuyers().size() + "," +
				best.getSellers().size() + ","
				+ best.getFitness() + ","+ this.lastEvolutionTime + "," + this.population.size() + "\n";
	...
	private Creature foundTrade() {
		...
		this.log.info("Best Gain at Interval ("+ this.apocalypse  + "):"  + best.getGain() + " Best Cost: " + best.getCost() + " Best Fitness: " + best.getFitness());

...
public class UsdTradeDebitManager extends Thread {
    ...
    private UsdBankSimulator usdBankSimulator;
    private final TradeService tradeService;

    public UsdTradeDebitManager(UsdBankSimulator usdBankSimulator, TradeService tradeService) {
    ...
    public void processTradeDebits() throws Exception {
            ...
            for (int j = 0; j < traders.size(); j++) {
                ...
                for (int k = 0; k < traders.get(j).getBeneficiaries().size(); k++) {
                    ...
                    long amount = traders.get(j).getBeneficiaries().get(k).getAmount().getValue()
                        / 100; //Value in Cents
                    totalDebits = totalDebits + amount; //Total them up
        ...
        if (debits.size() > 0) {
            ...
            String bsb = "dummy"; //UsdBankSimulator.getBsbNumber()
            NabFileTotalRecord fileTotal = new NabFileTotalRecord(bsb,

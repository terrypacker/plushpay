...
public class Population {
    ...
    public void crossoverFittest() throws Exception {
        ...
        for (int i = 0; i < numCrossovers; i++) {
            ...
            for (int j = 2; j < this.creatures.size(); j++) {
                ...
                } else if (this.creatures.get(j).getFitness() > creatures.get(secondBestPos)
                    .getFitness()) {
                    secondBestPos = j;
                }
            }
     ...
     * <p>
     * Return the percentage of the population that is fit.
     */
    ...
    public void populationControl() {
        ...
        if (this.creatures.size() > this.maxPopulationSize * 3) {
            ...
            this.unfitThreshold = this.best.getFitness() / ((float) this.creatures.size()
                / (float) this.maxPopulationSize);
            this.log.info("Population Blowout Detected, jacking up unfit threshold to: "
                + this.unfitThreshold);
            return;
        }

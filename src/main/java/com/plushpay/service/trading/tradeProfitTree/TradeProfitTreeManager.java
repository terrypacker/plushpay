...
public class TradeProfitTreeManager {
    ...
    /**
     * Insert new traders by collecting traders from DB and merge with existing list.
     *
     * @throws Exception
     */
    ...
    /**
     * Check to see if best match in the tree is a valid trade.
     *
     * @return
     */
    ...
    public void run() {
        ...
        this.maxComputationTime =
            100000000 * 2; //100000000; //500*1000000;//in nanoSeconds to insert one buyer
    ...
    private void adjustParameters(long computationTime, int treeSize) throws Exception {
        ...
        if (best != null) {
            buyerProfit = best.getBuyers().getCurrencyToSell().getValue() - best.getSellers()
                .getCurrencyToBuy().getValue();
            sellerProfit = best.getSellers().getCurrencyToSell().getValue() - best.getBuyers()
                .getCurrencyToBuy().getValue();
    ...
    private void writeParamData(double factor, long computationTime, int treeSize, long buyerProfit,
        long sellerProfit, float roi, int numBuyers, int numSellers, long tradeId)
        throws IOException {

        String out =
            tradeId + "," + numBuyers + "," + numSellers + "," + ((float) buyerProfit / 10000f)
                + "," + ((float) sellerProfit / 10000f) + "," + roi + "," + treeSize + ","

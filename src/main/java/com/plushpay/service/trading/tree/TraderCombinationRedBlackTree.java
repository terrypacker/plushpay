...
public class TraderCombinationRedBlackTree {
    ...
    public void insertSeller(Trader seller) throws Exception {
        ...
        this.log.info(
            "Building and Inserting " + startSize + " nodes took:" + ((System.nanoTime() - now)
                / 1000000) + "ms");
...
//		}
    ...
    public int trimTree(double delta) {
        ...
        while (it.hasNext()) {
            ...
            if (predecessor.getValue().getTotalMismatch() > (long) (trim * current.getValue()
                .getTotalMismatch())) {
    ...
    /**
     * Crop the tree by removing any node with mismatch above cropAbove% of the worst match.
     * <p>
     * This will not allow removal of the best group, so if the computed removal point is below best
     * group, we will move it to just above best group
     * <p>
     * The larger the value of cropAbove the more nodes removed from the tree.
     * <p>
    ...
    /**
     * Insert a buyer into the tree Crop and Trim Tree in the process
     * <p>
     * NOT FULLY TESTED
     *
     * @param buyer
    ...
    public void insertBuyer(Trader buyer, float delta, float cropAbove) throws Exception {
        ...
        List<TraderCombinationNode> newNodes = new ArrayList<TraderCombinationNode>(
            this.tree.size());
        ...
        if (this.tree.size() == 1) {
            ...
            if (newNode.getTotalMismatch() > cropAboveValue) {
                //Don't add it
            } else if (predecessor.getValue().getTotalMismatch() > (long) (trim
                * newNode.getTotalMismatch())) {
                //Dont add it
            } else {
                //We will Add the group
        ...
        System.out.println(
            "Building " + newNodes.size() + " nodes took:" + (System.nanoTime() - now));
        ...
        System.out.println(
            "Inserting " + newNodes.size() + " nodes took:" + (System.nanoTime() - now));
    }
        ...
        for (int i = 1; i < startSize; i++) {
            ...
            currentMismatch = (Math.abs(this.nodes.get(i).getBuyers().getCurrencyToBuy().getValue()
                ...
                Math.abs(this.nodes.get(i).getBuyers().getCurrencyToSell().getValue()
                    + buyer.getCurrencyToSell().getValue() - this.nodes.get(i).getSellers()
                    .getCurrencyToBuy().getValue()));
    ...
    /**
     * Put a node into the Tree, this also adds the node to the List of nodes that are in the tree,
     * to speed up some operations we keep a list AND a map.
     * <p>
    ...
    public void insertSeller(Trader seller, float cropAbove) throws Exception {
        ...
        for (int i = 1; i < startSize; i++) {
            ...
            currentMismatch = (Math.abs(this.nodes.get(i).getBuyers().getCurrencyToBuy().getValue()
                - this.nodes.get(i).getSellers().getCurrencyToSell().getValue()
                + seller.getCurrencyToSell().getValue()) +
                Math.abs(this.nodes.get(i).getBuyers().getCurrencyToSell().getValue() -
                    this.nodes.get(i).getSellers().getCurrencyToBuy().getValue()
                    + seller.getCurrencyToBuy().getValue()));
    ...
    public void insertSeller(Trader seller, float delta, float cropAbove) throws Exception {
        ...
        while (it.hasNext()) {
            ...
            currentMismatch = (Math.abs(current.getValue().getBuyers().getCurrencyToBuy().getValue()
                ...
                + seller.getCurrencyToSell().getValue()) +
                Math.abs(current.getValue().getBuyers().getCurrencyToSell().getValue() -
                    current.getValue().getSellers().getCurrencyToBuy().getValue()
                    + seller.getCurrencyToBuy().getValue()));
    ...
    /**
     * Trim Tree Using keys instead of a map Careful, this does NOT remove nodes from the nodes
     * list... :(
     *
     * @param delta
     */
    ...
    public void trimAndCropTree(float delta, float cropAbove) {
        ...
        while (it.hasNext()) {
            ...
            } else if (predecessor.getValue().getTotalMismatch() > (long) (trim * current.getValue()
                .getTotalMismatch())) {
                //Yes trim it
                //System.out.println("Removing node: " + current);
                //System.out.println("Because: " + predecessor + " > " + (long)((1-delta)*current));

...
public class TraderCombinationRedBlackTreeManager implements Runnable {
    ...
    /**
     * Insert new traders cropping any mismatch above within cropAbove% of the worst mismatch.
     *
     * @param cropAbove
     * @throws Exception
     */
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (best != null) {
                this.log.info("Best Group: " + best.getTotalMismatchInDollars() + " with " + (
                    best.getBuyers().size() + best.getSellers().size()) + " members.");
    ...
    /**
     * Adjust the tuning parameters to keep the algorithm running at best performance
     * <p>
     * If it runs quicker than the maxComputationTime, then the accuracy is increased.  If it runs
     * slower, then the accuracy is reduced.
     * <p>
     * If no more items to add and accuracy is as good as we can get then we should re-build the
     * tree in an attempt to get a match.
     * <p>
     * These adjustments should be dependent on tree size.
     * <p>
     * We linearize the computation time depending on the tree size. So we can more accurately
     * detect exponential blow-out of the tree.
     * <p>
     ...
     */
    private void adjustParameters(long computationTime, int treeSize, long mismatch, int numBuyers,
        int numSellers) throws Exception {

        //long computedRate = (long) (computationTime/treeSize);
        this.log.info(
            "Computation Time: " + computationTime + " Max Allowed is: " + this.maxComputationTime);
        ...
        if (computationTime < this.maxComputationTime) {
            ...
            if (this.adjustableTrimWithin > this.minTrimWithin) {
                ...
                this.adjustableTrimWithin =
                    this.adjustableTrimWithin - this.trimWithinDecrement * factor;

                if (this.adjustableTrimWithin < this.minTrimWithin) {
                    this.adjustableTrimWithin = this.minTrimWithin;
                }
                this.log.info(
                    "Reducing Trim Within from " + oldValue + " to " + this.adjustableTrimWithin);
            } else {
            ...
            if (this.adjustableCropAbove > this.minCropAbove) {
                ...
                this.adjustableCropAbove =
                    this.adjustableCropAbove - this.cropAboveDecrement * factor;

                if (this.adjustableCropAbove < this.minCropAbove) {
                    this.adjustableCropAbove = this.minCropAbove;
                }
                this.log.info(
                    "Reducing Crop Above from " + oldValue + " to " + this.adjustableCropAbove);
            } else {
			...
			*/
            ...
            if (this.adjustableTrimWithin < this.maxTrimWithin) {
                ...
                oldValue = this.adjustableTrimWithin;
                this.adjustableTrimWithin =
                    this.adjustableTrimWithin + this.trimWithinIncrement * factor;

                if (this.adjustableTrimWithin > this.maxTrimWithin) {
                    this.adjustableTrimWithin = this.maxTrimWithin;
                }
                this.log.info(
                    "Increasing Trim Within from " + oldValue + " to " + this.adjustableTrimWithin);
            } else {
            ...
            if (this.adjustableCropAbove < this.maxCropAbove) {
                ...
                oldValue = this.adjustableCropAbove;
                this.adjustableCropAbove =
                    this.adjustableCropAbove + this.cropAboveIncrement * factor;

                if (this.adjustableCropAbove > this.maxCropAbove) {
                    this.adjustableCropAbove = this.maxCropAbove;
                }
                this.log.info(
                    "Increasing CropAbove from " + oldValue + " to " + this.adjustableCropAbove);
            } else {
            ...
            this.log.info(
                "Trimming Tree took: " + ((System.nanoTime() - nanoNow) / 1000000) + "ms");
            this.log.info("Tree size now: " + this.tree.size());
            ...
            if (computationTime > 2 * this.maxComputationTime) {
                ...
                this.log.info(
                    "Disaster Control, tree size: " + this.tree.size() + " Cropping Tree by: "
                        + this.adjustableCropAbove);
                this.tree.cropTree(this.adjustableCropAbove); //Hack off a big section of tree
                this.log.info("Tree size now: " + this.tree.size());
            }
     ...
     * <p>
     * numBuyers,numSellers,bestMismatch,treeSize,
     * computationTimeNs,computationTimeMs,computationTimeS
     ...
     * @throws IOException
     */
    private void writeParamData(double factor, long computationTime, int treeSize,
        long bestMismatch, int numBuyers, int numSellers) throws IOException {

        String out =
            numBuyers + "," + numSellers + "," + ((float) bestMismatch / 10000f) + "," + treeSize
                + "," + computationTime + "," + (computationTime / 1000000) + "," +
    ...
    private void logCurrentParameters() {
        ...
        this.log.info("Trim By Removing anyone within: " + (
            (1f - (this.adjustableTrimWithin / (float) this.tree.size())) * 100f)
            + "% of previous member. " + "(" + this.adjustableTrimWithin + ")");
        this.log.info(
            "Crop Any Group with Mismatch above: " + ((1 - this.adjustableCropAbove) * 100f)
                + "% of Worst Group. " + "(" + this.adjustableCropAbove + ")");
    ...
    public void insertSellers(List<Trader> sellers) throws Exception {
        ...
        for (int i = 0; i < sellers.size(); i++) {
            ...
            TraderCombinationNode best = this.tree.getBest();
            this.log.info(
                "Best Group: " + best.getTotalMismatch() + " with " + (best.getBuyers().size()
                    + best.getSellers().size()) + " members.");
        }
    ...
    public void insertSeller(Trader newSeller, int maxSize) throws Exception {
        ...
        for (int i = 0; i < startSize; i++) {
            ...
            if (nodes.get(i).getSellers().size() < maxSize - 1) {
                ...
                long mismatch = Math.abs(
                    nodes.get(i).getBuyers().getCurrencyToBuy().getValue() - nodes.get(i)
                        .getSellers().getCurrencyToSell().getValue());
                long bestMismatch = 0;
                long thisMismatch = 0;
                ...
                if (mismatch != 0) {
                    //decrease
                    bestMismatch = Math.abs(
                        mismatch - newSeller.getCurrencyToSell().getValue() + newSellersGroup.get(0)
                    ...
                    for (int j = 1; j < newSellersGroup.size(); j++) {
                        ...
                        thisMismatch = Math.abs(mismatch - newSeller.getCurrencyToSell().getValue()
                            + newSellersGroup.get(j).getCurrencyToSell().getValue());
        ...
        for (int i = 0; i < startSize; i++) {
            ...
            if (nodes.get(i).getBuyers().size() < maxSize - 1) {
                ...
                if (mismatch != 0) {
                    //decrease
                    bestMismatch = Math.abs(
                        mismatch + newBuyer.getCurrencyToBuy().getValue() - newBuyersGroup.get(0)
                    ...
                    for (int j = 1; j < newBuyersGroup.size(); j++) {
                        //We want to replace a buyer so that
                        //Sellers->selling - Buyers->buying = newBuyer - oldBuyer
                        thisMismatch = Math.abs(
                            mismatch + newBuyer.getCurrencyToBuy().getValue() - newBuyersGroup.get(
                                j).getCurrencyToBuy().getValue());
                        if (thisMismatch < bestMismatch) {
                            bestMismatch = thisMismatch;
                            replace = j;
                        }

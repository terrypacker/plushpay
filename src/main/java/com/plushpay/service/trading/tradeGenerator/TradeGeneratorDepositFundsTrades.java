...
public class TradeGeneratorDepositFundsTrades {
    ...
    public TradeGeneratorDepositFundsTrades(TradeRepository tradeRepository) {
        this.tradeRepository = tradeRepository;
    }

    public void init() {

        this.setTrades(tradeRepository.getAllWithStatus(TradeStatus.DEPOSIT).toList());
        ...
        if (this.trades.size() > 0) {
            ...
            this.setTotalBuyerSellMismatch(new PyCurrency(0,
                new PyCurrencyType(CurrencyCodeEnum.USD, 0, ZonedDateTime.now())));
            this.setTotalSellerBuyMismatch(new PyCurrency(0,
                new PyCurrencyType(CurrencyCodeEnum.USD, 0, ZonedDateTime.now())));
            this.setTotalSellerSellMismatch(new PyCurrency(0,
                new PyCurrencyType(CurrencyCodeEnum.USD, 0, ZonedDateTime.now())));

...
public class ExactTradeManager extends Thread {
    ...
    public void run() {
        ...
        while (!this.shutdown) {
            ...
            try {
                Thread.sleep(
                    this.lastComputationTime / 4); //Duty cycle of 75% (To regulate CPU work)
    ...
    private void createTrade(Combination best) {
        ...
        long buyerProfit =
            buyersGroup.getCurrencyToSell().getValue() - sellersGroup.getCurrencyToBuy().getValue();
        long sellerProfit =
            sellersGroup.getCurrencyToSell().getValue() - buyersGroup.getCurrencyToBuy().getValue();
        this.log.debug(
    ...
    /**
     * Compute the solution using the available traders in numThreads number of threads.
     *
     * @throws
     */
    private void computeSolutionThreaded() {
        ...
        try {
            ...
            for (int i = 0; i < numThreads; i++) {
                ExactTradeThread thread = new ExactTradeThread(
                    this.currentBuyers.subList(this.buyerStartPos, this.buyerEndPos),
                ...
                if (this.buyerEndPos >= this.currentBuyers.size()) {
                    ...
                    this.buyerStartPos =
                        this.buyerStartPos - 1; //Step back one to stay in same place
                } else {
                    if (this.sellerStartPos == 0) {
    ...
    /**
     * Compute one solution from a selection of buyers and sellers.
     */
    private void computeSolution() {

        try {
            long now = System.nanoTime();
            this.gen = new TradeCombinationGenerator(
                this.currentBuyers.subList(this.buyerStartPos, this.buyerEndPos),
                this.currentSellers.subList(this.sellerStartPos, this.sellerEndPos));
    ...
    /**
     * Search the available Traders and find a grouping that contains no doubles of rates
     * buyer-seller
     *
     * @return
     */
    ...
    /**
     * Using maxCost and minGain determine if best is good enough for a trade.
     *
     * @return
     */
    private Combination foundTrade() {
        ...
        this.log.info(
            "Best Gain:" + best.getGain() + " Best Cost: " + best.getCost() + " Best ROI: "
                + best.getRoi()

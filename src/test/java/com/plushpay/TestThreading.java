...
import com.plushpay.service.currency.rate.RateCollectorThread;
...
public class TestThreading extends ServletTestCase {
    ...
    public void testStartup() {
        ...
        assertEquals(State.NEW,
            AudTraderDepositSimulator.getAudTraderDeptositSimulator().getState());
        ...
        assertTrue(
            (State.RUNNABLE == RateTestDataCollector.getRateTestDataCollector().getState()) ||
                (State.BLOCKED == RateTestDataCollector.getRateTestDataCollector().getState()));

        assertTrue(
            (State.RUNNABLE == AudTradeCreditManager.getAudTradeCreditManager().getState()) ||
        ...
        assertTrue(
            (State.RUNNABLE == AudTraderDepositSimulator.getAudTraderDeptositSimulator().getState())
                ||
                (State.BLOCKED == AudTraderDepositSimulator.getAudTraderDeptositSimulator()
                    .getState()));

        assertTrue(
            (State.RUNNABLE == TradeProfitTreeManager.getTradeProfitTreeManager().getState()) ||
    ...
    public void testShutdown() {
        ...
        assertEquals(State.TERMINATED,
            AudTraderDepositSimulator.getAudTraderDeptositSimulator().getState());
        assertEquals(State.TERMINATED,
            TradeProfitTreeManager.getTradeProfitTreeManager().getState());
